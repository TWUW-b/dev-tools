 ...
 */
export function computePipPosition({
  pipWidth,
  pipHeight,
  screenAvailWidth,
  screenAvailHeight,
  initialPosition,
  margin = 20,
}: PipPositionInput): { x: number; y: number } {
  if (initialPosition) return initialPosition;

  return {
    x: Math.max(0, screenAvailWidth - pipWidth - margin),
    y: Math.max(0, screenAvailHeight - pipHeight - margin),
  };
}
