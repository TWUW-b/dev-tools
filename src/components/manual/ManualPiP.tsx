...
export function ManualPiP({
  ...
  const openPipWindow = useCallback(async () => {
    ...
    try {
      ...
      // Picture-in-Picture API の仕様上の意図的な制限。なりすまし防止のため
      // "The website cannot set the position of the PiP window" と明記されている。
      // https://wicg.github.io/document-picture-in-picture/ 、
      // https://github.com/WICG/document-picture-in-picture/issues/34）。
      // pip.moveTo() は Window オブジェクトとして型上は呼べるが、Chrome では
      // 黙って無視される。width/height（サイズ）はサイト側から指定可能。
