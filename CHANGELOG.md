...
- **マニュアルにアプリ本体のアイコンを出せるように**: マニュアルは Markdown なので React
  コンポーネントを直接書けず、アプリ側のアイコン（lucide 等）をそのまま使う手段が無かった。
  「名前 → ノード」の対応表を渡しておき、本文には `<app-icon name="building"></app-icon>` と
  書くだけで置き換わる仕組みを追加した。SVG をマニュアルへコピーする必要がなく、アプリ側で
  アイコンを差し替えればマニュアルも追従する。
  - `MarkdownRenderer` に `icons` を追加。`ManualPiP` / `ManualTabPage` / `DebugPanel`
    （`manualIcons`）から渡すと本文に反映される。ManualPiP（別ウィンドウ）でも動作する
  - 既定では装飾扱い（`aria-hidden`）。`label` を指定した場合だけ `role="img"` +
    `aria-label` で読み上げ対象になる。未登録の name・`icons` 未指定時は何も描画しない
  - ラッパーに `manual-icon` クラスを付与（位置調整用の CSS フック）
  - NOTE: MD には閉じタグ付きで書く。`<app-icon … />` は HTML のパース規則上「開始タグ」
    として扱われ、以降がアイコンの子要素になってしまう
- **目次・サイドバーのアイコン**: `ManualItem.icon` でページ行に、`categoryIcons`
  （`ManualSidebar` / `ManualTableOfContents` / `ManualPiP` / `ManualTabPage`）で
  カテゴリ見出しにアイコンを出せるようにした。アイコンが無い項目の見た目は従来どおり。

### Fixed

- **見出しにアイコン（生 HTML）を入れると目次からジャンプできなくなる不具合を修正**:
  本文側の id は rehype-slug が「レンダリング後のテキスト」から作るのに対し、目次側
  （`useManualHeadings`）は Markdown ソースから作っており、生 HTML を除去していなかった。
  その結果 `## <app-icon …></app-icon> 新しい物件情報が入ったとき` は本文側
  `-新しい物件情報が入ったとき` に対して目次側が `svg-xmlnshttpwwww3org2000svg-…` となり、
  目次のリンクが機能しないうえ目次に HTML ソースがそのまま並んでいた。生 HTML タグを除去し、
  かつ除去跡の空白を残す（＝ trim しない）ことで rehype-slug と同じ id を生成するようにした。
  画像だけの見出し（`## ![](icon.svg) 物件`）も同じ理由でズレていたため、あわせて直っている。
- **アイコンサイズの画像がクリック拡大の対象になっていた**: v1.4.12 で追加した画像の
  クリック拡大が 18px のアイコン画像にも効き、全画面のライトボックスが開いていた。
  幅・高さを明示していて最大辺 48px 以下の画像、および `data-no-zoom` を付けた画像は
  拡大の対象外にした。
...
- **マニュアル本文の画像をクリックで拡大表示できるように**: マニュアルの画像はスクリーンショットが
  主で、本文の幅に収まった状態では画面内の文字やボタンが読めなかった。`MarkdownRenderer` が描画する
  画像（Markdown の `![]()` と、生 HTML の `<img>` の両方）をクリックすると、画面全体を覆う
  ライトボックスで拡大表示する。背景クリック / × ボタン / Escape で閉じ、閉じると元の画像へ
  フォーカスが戻る。「実寸で表示」で等倍表示に切り替えでき（画像自身のクリックでも切り替わる）、
  はみ出す分はスクロールできる。キャプションは `title` 属性、無ければ `alt` を使う。
  `MarkdownRenderer` を使う全ての面（`ManualPiP` / `ManualTabPage` / `ManualSidebar` /
  `DebugPanel` のマニュアルタブ / `EnvironmentTab`）に同時に効く。
  `disableImageZoom` で無効化できる（既定は有効）。
  - **画像に重ねた注記も拡大表示に引き継ぐ**: 「ここを押す」を四角で囲うタイプの手順表示
    （`.manual-shot` の中に `<img>` と絶対配置の `.manual-mark` を並べる構成）では、拡大したときに
    囲みが消えるとどこを指しているのか分からなくなる。画像の親要素に絶対配置の兄弟要素がある場合は
    それを注記とみなし、拡大画像の上にも同じ位置・比率で複製する（注記の位置指定は % なので、
    拡大率にそのまま追随する）。
  - **`<img>` はラッパー要素で包まない**: ホスト側のマニュアル CSS は
    `.manual-shot img { width: 100% }` のように「コンテナ直下の img」を前提に書かれているため、
    間に要素を挟むと画像の幅の基準が変わり、重ねた注記の位置がずれる。クリックの当たり判定は
    `<img>` 自身に持たせ、DOM 構造は従来どおりにしている。
  - `ImageLightbox` として単体でも公開（`ReleaseNotes` の画像拡大もこの実装に統一した）。
    `position: fixed` で描画先の document を覆う実装、Escape とスクロールロックも描画先の
    `ownerDocument` に対して行うため、`ManualPiP`（Document Picture-in-Picture の別ウィンドウ）
    の中でもそのウィンドウ内で正しく開く。

### Fixed

- **`MarkdownRenderer` の再描画で本文ツリーが毎回作り直されていた問題を修正**: react-markdown に
  渡す `components` を毎レンダー新しいオブジェクト（＝新しいコンポーネント型）で作っていたため、
  React が別コンポーネントとみなして本文全体をアンマウント→再マウントしていた。開いていた
  `<details>` が閉じる・フォーカスが外れるといった副作用があるため、`useMemo` で固定した。

### Tests

- Unit: 156 / API: 135 (+1 skipped) / E2E: 19

## [1.4.11] - 2026-09-08

### Added

- **`ManualPiP` のヘッダーに戻るボタンを追加**: PiP は独立ウィンドウでブラウザの戻る操作が
  効かないため、目次や本文中のリンクで別ページへ移ったあと、直前に読んでいたページへ戻るには
  目次から辿り直すしかなかった。ヘッダー左端 (目次ボタンの隣) に戻るボタンを置き、PiP 内の
  ページ履歴を 1 つ戻れるようにした。戻り先が無いときは disabled (薄く表示)。戻る操作で
  発生した `docPath` の変化は履歴に積み直さないので、同じ 2 ページを往復しない。
  PiP を閉じると履歴は破棄する。
...
- **`ManualItem` に `hideHeadingsOutline` プロパティを追加し、目次サイドバー内のカテゴリ名
  二重表示を解消**: TOP ページ（本文が「今やりたいことを選んでください」的な案内文で、
  各セクションの見出し文言が実際のカテゴリ名と同じになっている想定）で、サイドバーの
  「見出しを開く」トグルを開くと、本文の h2/h3 見出し一覧（＝カテゴリ名）と、その直下に
  実際に並ぶカテゴリボタン（＝同じカテゴリ名）が並んで表示され、同じ文言が二重に見える
  問題があった。`hideHeadingsOutline: true` を指定した項目は見出しトグル自体を出さない
  ようにして解消（本文側の見出しはそのまま残るため、可読性・アクセシビリティは変わらない）。
  デフォルト `false`（未指定）は既存挙動のまま変わらない。

## [1.4.9] - 2026-08-31

### Fixed

- **`ManualTabPage`（`/manual-view` 別ページ）の本文 `app:` リンクが、`window.open()` 経由
  以外で開いた場合に無反応だった不具合を修正**: 本文 app: リンクのクリックハンドラは
  `window.opener` への `postMessage({ type: 'manual-app-navigate', path })` のみを行っており、
  ホスト側は「MainLayout から `window.open()` で開かれ、`window.opener` が呼び出し元を指している」
  ことを前提にこれを受信していた。`/manual-view` を直接 URL アクセス・ブックマーク・新規タブで
  開いた場合は `window.opener` が存在せず、postMessage の送信自体が実行されず、クリックしても
  完全に無反応になっていた。新しい `onAppNavigate` プロパティ（`ManualPiP` と同名で揃えた）を
  `window.opener` が無い場合のフォールバックとして呼び出すよう修正。既存の
  `window.open()` 経由での postMessage 動作は変更していない。

### Added

- **`ManualTabPage` / `ManualTableOfContents` に `defaultExpandCategories` プロパティを追加**:
  目次サイドバーのカテゴリ初期開閉状態を `'active'`（デフォルト、既存挙動＝現在表示中の
  ページを含むカテゴリのみ開く）と `'all'`（全カテゴリを初期状態から展開）から選べるように
  した。未指定時は既存挙動のまま変わらない。
- **`ManualPiP` の目次パネルをハンバーガーボタン/パネル本体へのホバーで開閉できるように**:
  従来はハンバーガーボタンのクリックでのみ開閉していた。ボタンまたはパネル本体にマウスが
  乗っている間は開いたままにし、両方から離れると 200ms 待って自動的に閉じる（ボタン→パネルへ
  ポインタを移動する一瞬のチラつきを防止するための遅延）。クリックでの開閉は残しているが、
  ホバーで既に開いている状態でクリックすると即座に閉じてしまう問題を避けるため、クリックは
  トグルではなく常に「開く」動作に変更した（閉じる手段は閉じるボタン・背景クリック・Escape・
  ホバー解除で従来どおり担保）。
...
- **PiPハンバーガーメニューのアイコン色が黒のままになる不具合を修正**: `.pip-menu-btn`
  (目次パネル開閉ボタン) に `color` 指定が漏れており、`button` 要素の UA デフォルト
  スタイルで中の `.pip-icon` (Material Symbols の "menu" リガチャ) が黒文字のまま
  描画されていた。紺色ヘッダー背景で視認性が低下していた。兄弟の `.pip-close-btn` /
  `.pip-download-btn` には `color: COLORS.white` が既に明示指定されており、
  `.pip-menu-btn` だけ指定漏れだった。同パターンに揃えて追加。

## [1.4.6] - 2026-08-28

### Docs

- **Document Picture-in-Picture の `width`/`height` 実測結果を `initialSize` の JSDoc に追記**。
  実装コードの変更はなし。Chrome 実機（バージョン151・macOS・DPR2・
  画面1680×1050）で `requestWindow({width, height, preferInitialWindowPlacement: true})`
  を26パターン(width方向14件・height方向12件)実測して得た規則を記録する。

  **width（height=400固定でスイープ）:**

  | 指定値 | 実際の innerWidth |
  |---|---|
  | 100〜239 | 常に **240**（Chromium の最小幅 `kMinWindowSize.width`） |
  | 241〜800 | 指定値そのまま |
  | 1200〜2000 | 縮小される（画面サイズ由来の上限。詳細は後述） |

  `outerWidth = innerWidth + 8`（常に一定、ウィンドウ枠分と推定）。

  **height（width=400固定でスイープ）:**

  | 指定値 | 実際の outerHeight | 実際の innerHeight |
  |---|---|---|
  | 52 | 58 | 52（最小値扱い、下記式に従わない特殊値） |
  | 80〜600 | **指定値 − 50** | 指定値 − 56 |
  | 800以上 | 702 で頭打ち | 696 で頭打ち |

  例: `height: 275` → 実際は outer 225 / inner 219。`height: 600` → outer 550 / inner 544。
  この「−50（outer）/ −56（inner）」というオフセットは 80〜600 の範囲で一貫していた
  （PiP 専用ヘッダーの高さ分と推定、公式ドキュメントに明記はない）。

  **実務上の指定方法:** 欲しい viewport の高さ（`innerHeight`）を得たい場合は
  `height = 欲しい高さ + 56` を指定する。width は 240px 以上を指定すればそのまま反映され、
  240px 未満は指定しても無意味（240 になる）。

  **過大要求時の相互作用:** width か height の一方が画面サイズに対して過大だと、
  指定していない方の辺も連動して縮小される（例: `width:400, height:1500` を要求すると
  width も 240 相当まで潰れる）。正確な計算式（面積制約かアスペクト比制約か）は未特定。

  **位置は全パターンで不変:** `screenX`/`screenY` は width/height をどう変えても
  常に同一値だった（今回の実機では `(1, 7)`、画面左上付近）。[1.4.3] で確定した
  「位置はサイト側から制御不可能」という結論を裏付ける追加の実証データ。

  検証環境の制約: これらの数値はテストした1環境（Chrome 151・macOS・DPR2）での実測値で、
  OS・ブラウザバージョン・ディスプレイ構成が変われば変わりうる。実装で厳密に依存しない設計にすること。
